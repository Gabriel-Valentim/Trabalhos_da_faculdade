...
void salvarArquivo(int *i){

   if((fp = fopen("LocadoraValpa.txt","w+b")) == NULL){
    printf("O arquivo nao pode ser aberto");
    exit(1);
    }

    for(int j = 0; j < *i; j++){
        fwrite(&cadastro[j],sizeof(struct cadastro_Filmes),1,fp);
    }
    fclose(fp);
}
...
void lerArquivo(){
    ...
    printf("\n-----Banco de Dados Locadora Valpa-----\n");
    
    printf("\nVoce ja tem filmes salvos no banco de dados ?");
    printf("digite 1 caso tenha e 0 caso nao tenha:\n");
    scanf("%i", &aux);

    if (aux == 1){

        if((fp = fopen("LocadoraValpa.txt","r+b")) == NULL){
            printf("O arquivo nao pode ser aberto");
            exit(1);
        }

        printf("Quantos filmes ja estao cadastrados no banco de dados: ");
        scanf("%i", &quant);

        for (int i = 0; i < quant; i++){
            fread(&cadastro[i],sizeof(struct cadastro_Filmes),1,fp);
        }
        fclose(fp);
    } 
...
void atualizar(int *i){
    ...
    printf("Qual codigo do filme que voce deseja atualizar o preco: ");
    scanf("%i", &atual);
